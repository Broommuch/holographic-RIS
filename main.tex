...
holographic MIMO reception from intensity-only measurements. A controllable RF
reference is superimposed element-wise with the incident field before
...
stability bounds. A four-phase reference construction provides explicit field
recovery and reveals that severe reference--signal energy imbalance weakens
phase information. Guided by these results, a two-stage receiver combines
practical field-based initialization, stability-guided adaptive training,
...
conditions and show that, at $8$~dB, the adaptive policy meets a $-25$~dB
channel-NMSE target using $16.5$ pilot measurements on average instead of the
fixed budget of $40$, while the proposed distance metric accurately predicts
...
Phaseless recovery has also appeared in communication problems such as OFDM channel estimation \cite{ofdm_phase_retrieval}. For holographic interference surfaces, Huang et al. developed phase-shifting interference suppression and Prony-based multi-user channel segmentation by exploiting the spatial sum-of-exponentials structure \cite{huangjd2024CS}; a subsequent wideband extension introduced holographic channel inversion together with maximum-likelihood and low-complexity sensing algorithms \cite{huang2026wideband}. As with Prony-type estimators generally, the finite-aperture robustness of the former depends on the conditioning of its pole-recovery and Vandermonde systems, particularly for closely spaced spatial frequencies. Meanwhile, RIS channel acquisition and codebook design have been studied for conventional complex-baseband observations \cite{Zhou2024,r1,r3}. These works establish useful channel-sensing and training mechanisms, but they do not characterize the finite-measurement injectivity and stability of the complete affine-intensity map or resolve the identifiability of finite-alphabet symbols. In particular, generic continuous-signal guarantees do not directly describe when two constellation vectors produce identical or nearly identical energy patterns under the coupled effects of the reference wave, waveform sampling, and RIS array manifold.
