...
\begin{figure}[t]
    ...
    \caption{Joint DOA RMSE and the corresponding CRLB versus SNR under the fixed spatial four-phase reference code. Error bars show 95\% confidence intervals.}
...
and LM refinement. The experiment uses two users at polar angles
...
while the linearized and GS detectors trade accuracy for lower complexity. At
$-5$~dB, the ML and certified-GS SERs are $0.161$ and $0.211$, respectively. The analytical
distance-spectrum union bound from Theorem~\ref{thm:discrete_stability} is
$0.333$ and approaches the ML curve in the waterfall region.
...
$25$-iteration budget. Its average number of back-projections falls to $4.05$
at $11$~dB and essentially zero at $13$~dB, once the initialized candidate lies
inside the exact decoding radius. Thus, the distance analysis supplies a
rigorous stopping rule rather than an empirical residual threshold.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{simulation_results/ch5_spatial_reference_consistent_2026/results/fig_spatial_certified_gs.eps}
    ...
    (b) average GS back-projections with a fixed budget and the exact
    $d_{\min}^{(E)}$ certificate.}
...
each of the $300$ channel trials uses $200$ independent data vectors. The
...
\subsection{Effect of Spatial Sampling and Angular Separation}

We next examine how receiving-surface sampling and angular separation change the
...
\begin{figure}[t]
    ...
    \caption{Effect of the number of receiving units on (a) the normalized energy-domain minimum distance and (b) perfect-CSI ML detection at $-12$~dB.}
...
Together, these results show that spatial sampling and array-manifold conditioning
...
balanced fixed spatial phase maps, with $1500$ symbol trials per map and ratio.
...
\begin{figure}[t]
    ...
    \caption{Effect of the reference-to-signal energy ratio under a fixed total incident-energy budget: (a) ensemble-mean minimum energy distance over balanced spatial phase maps and (b) perfect-CSI ML SER at three balanced-case SNRs. Error bars show one standard deviation across phase maps.}
...
This paper developed a reference-assisted RF holographic receiver that recovers
...
spatial four-phase coding provided an ensemble-mean separation guarantee. A damped
...
as benchmarks. Simulations confirmed that spatial reference coding, pilot
diversity, surface size, and angular separation jointly determine estimation and
