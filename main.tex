...
\end{theorem}
\begin{proof}
The claim follows directly from Lemma~\ref{lem:midpoint_factorization}.
\end{proof}
...
\end{corollary}
\begin{proof}
Expanding the energy difference gives
$2\mathbf a_{ij}+2\operatorname{Re}\{\mathbf b^*\odot\mathbf d_{ij}\}$.
For $d_{ij,q}\ne0$, the ambiguous values are
$b_q=(-a_{ij,q}/|d_{ij,q}|^2+j\tau_q)d_{ij,q}$,
$\tau_q\in\mathbb R$, which impose one real affine constraint per active
coordinate. Thus, each $\mathcal B_{ij}^{\rm bad}$ is proper, and their finite
...
\begin{corollary}[Noise-limited finite-alphabet reliability]
