...
To test whether the Jacobian analysis can directly improve the estimator,
Fig.~\ref{fig:stability_aware_gn} compares undamped GN, conventional
Levenberg--Marquardt (LM) damping, and the stability-aware rule in
\eqref{eq:stability_aware_gn_damping}.  This stress test uses $T_{\rm p}=16$
identical user pilots, an SNR of $12$~dB, DOAs centered at $20^{\circ}$ with a
common $5^{\circ}$ azimuth, and $300$ paired trials from the same perturbed
initialization.  Recovery is declared successful when the joint DOA RMSE is
below $1^{\circ}$.  At separations of $0.5^{\circ}$ and $4^{\circ}$, the
stability-aware method attains success probabilities of $0.957$ and $0.943$
and channel NMSEs of $-18.42$ and $-18.05$~dB, respectively; the corresponding
LM results are $0.283$ and $0.397$, with NMSEs of $-0.06$ and $-6.14$~dB.
When the separation reaches $16^{\circ}$, all three methods coincide.  The
stability floor therefore suppresses unreliable steps only when the local
Jacobian is poorly conditioned, without changing the well-conditioned regime.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{simulation_results/ch5_theory_guided_algorithms_2026/results/fig_stability_aware_gn_accuracy_convergence.eps}
    \caption{Effect of inter-user angular separation on (a) channel NMSE and
    (b) successful recovery probability for undamped GN, conventional LM, and
    stability-aware GN.  The stress test uses $T_{\rm p}=16$, an SNR of
    $12$~dB, and $300$ paired trials; success denotes a joint DOA RMSE below
    $1^{\circ}$.}
    \label{fig:stability_aware_gn}
\end{figure}
...
distance-spectrum union bound from Corollary~\ref{cor:discrete_stability} is
...
We next examine how spatial and reference-state diversity change the
...
Together, these results support the two-part design principle in
