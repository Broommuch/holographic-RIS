20260521
# Section II. System Model

## A. Transmitter Model

## B. Far-Field RIS Channel Model

## C. Reference-Assisted Holographic Reception

## D. Two-Phase Transmission Protocol


# Section III: Structural Analysis of Reference-Assisted Energy Detection

## A. Unified Reference-Assisted Phase Retrieval Model

## B. Ambiguity Removal by Reference Wave

## C. Identifiability for Channel Estimation and Symbol Detection

## D. Energy-Domain Minimum Distance


# Section IV. Reference Wave and Measurement Design

## A. Reference Phase Diversity

## B. ~~Pilot Design for Channel Estimation~~  Reference Design for Channel Estimation

## C. ~~RIS Measurement Diversity~~ Reference Design for Symbol Detection

## D. Joint Reference and Measurement Diversity


## Section V. Channel Estimation and Data Detection Algorithms

## A. ~~Pilot-Based Channel Estimation~~  Overall Two-Phase Recovery Framework

## B. ~~Data Symbol Detection~~ Reference-Assisted Channel Estimation

## C. ~~Impact of Channel Estimation Error~~  Reference-Assisted Channel Estimation 

## D. Impact of Channel Estimation Error and Complexity Analysis (add)


# Section VI. Simulation Results

## A. Channel Estimation Performance

## B. Symbol Detection Performance with Perfect CSI
先验证理论极限：
ML；
GS；
GN；
no reference；
constant reference；
random reference；
uniform reference。

## C. Symbol Detection Performance with Estimated CSI

## D. Effect of Reference and RIS Measurement Design


# Section VII. Conclusion
总结：

提出了 reference-assisted holographic RIS receiver；
建立了统一的训练与数据相位恢复模型；
分析了信道估计和符号检测的可辨识性；
证明或说明参考波、导频、RIS 测量结构共同决定恢复性能；
通过仿真验证两阶段通信链路。