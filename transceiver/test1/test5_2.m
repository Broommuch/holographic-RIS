...
% CRB
theta_crb = zeros(length(SNR_dB_list),1);
phi_crb   = zeros(length(SNR_dB_list),1);
theta_crb_noref = zeros(length(SNR_dB_list),1);
...
for snr_idx = 1:length(SNR_dB_list)
    ...
    for n = 1:Nsym
        ...
        % ===== CRB (with reference wave) =====
        [~, J] = residual_jacobian(theta_hat, I, y, z, b, alpha, k);
        FIM = (1/noise_var) * (J.'*J);
        CRB = inv(FIM);
        
        theta_crb(snr_idx) = theta_crb(snr_idx) + CRB(3,3);
        phi_crb(snr_idx)   = phi_crb(snr_idx)   + CRB(4,4);
    ...
    %% CRB
    theta_crb(snr_idx) = theta_crb(snr_idx) / Nsym;
    phi_crb(snr_idx)   = phi_crb(snr_idx)   / Nsym;

    %% 
    % ===== CRB without reference wave =====
    b0 = 0;
    FIM0 = zeros(4);
    
    for i = 1:K
        phase = exp(1j*(ky*y(i)+kz*z(i)));
        h = alpha*s_true*phase;
        
        % 此时 mu_i = |h|^2，与角度无关
        % 对 theta, phi 的导数为 0
    end
    
    theta_crb_noref(snr_idx) = 1;   % 不可估
...
% CRB
figure;
semilogy(SNR_dB_list, theta_mse*180^2/pi^2, 'o-','LineWidth',1.5); hold on;
semilogy(SNR_dB_list, theta_crb*180^2/pi^2, '--','LineWidth',1.5);
grid on;
xlabel('SNR (dB)');
ylabel('Angle MSE / CRB (deg^2)');
legend('GN Estimation','CRB (with reference)');
title('Angle Estimation Performance');

figure;
semilogy(SNR_dB_list, theta_crb, 'r--','LineWidth',1.5);
hold on;
semilogy(SNR_dB_list, theta_crb_noref, 'k:','LineWidth',1.5);
grid on;
xlabel('SNR (dB)');
ylabel('CRB');
legend('With Reference','Without Reference');
title('Effect of Reference Wave on Identifiability');
