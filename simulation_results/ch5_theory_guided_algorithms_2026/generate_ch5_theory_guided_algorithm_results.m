%% Theory-guided algorithm simulations for Chapter V
% This self-contained script evaluates the two mechanisms introduced in
% Section IV: Jacobian-conditioned damping for parametric channel recovery
% and the energy-distance certificate for finite-alphabet GS detection.
% It also combines them in an end-to-end experiment.  EPS, PNG, FIG, CSV,
% and MAT outputs are written to ./results.
...
fprintf('\nTheory-guided Chapter V simulations\n');
...
gn_cfg.initial_angle_std_deg = 1.5;
gn_cfg.initial_gain_std = 0.10;
...
% Identical user pilots deliberately make the close-angle cases depend on
% spatial separation; this exposes the Jacobian conditioning mechanism.
pilot_symbols = make_correlated_qpsk_pilots( ...
    gn_cfg.num_pilots, gn_cfg.num_users, 1.0, 20260919);
training_reference = make_training_reference(gn_cfg);
...
for i_sep = 1:num_separations
    ...
    for mc = 1:gn_cfg.num_monte_carlo
        ...
        eta_initial = perturb_eta(eta_true, gn_cfg);
    ...
    fprintf(['GN separation %4.1f deg: success [%.3f %.3f %.3f], ', ...
        'RMSE [%.3f %.3f %.3f] deg\n'], separation_deg(i_sep), ...
...
e2e_cfg.initial_angle_std_deg = 1.5;
e2e_cfg.initial_gain_std = 0.10;
training_snr_dB = 0:4:20;

pilot_e2e = make_correlated_qpsk_pilots( ...
    e2e_cfg.num_pilots, e2e_cfg.num_users, 1.0, 20260920);
reference_training_e2e = make_training_reference(e2e_cfg);
...
for i_snr = 1:numel(training_snr_dB)
    ...
    for channel_trial = 1:e2e_cfg.num_channel_trials
        ...
        eta_initial = perturb_eta(eta_true_e2e, e2e_cfg);
...
save(fullfile(result_dir, 'theory_guided_algorithm_results.mat'), ...
    'cfg', 'gn_cfg', 'det_cfg', 'e2e_cfg', 'separation_deg', ...
    'success_probability', 'doa_rmse_deg', 'channel_nmse_dB', ...
    'average_gn_iterations', 'average_rejections', ...
    'snr_detection_dB', 'snr_complexity_dB', 'ser_ml', 'ser_linear', ...
    'ser_continuous_gs', 'ser_certified_gs', 'ser_union_bound', ...
    'avg_iter_fixed', 'avg_iter_exact', 'avg_iter_lower', ...
    'dmin_exact', 'dmin_lower', 'training_snr_dB', ...
...
%% Local functions
function S = make_correlated_qpsk_pilots(num_pilots, users, ...
        shared_fraction, seed)
    old_rng = rng;
    rng(seed, 'twister');
    qpsk = [1 + 1j; -1 + 1j; -1 - 1j; 1 - 1j] / sqrt(2);
    base = qpsk(randi(4, num_pilots, 1));
    S = repmat(base, 1, users);
    for user = 2:users
        replace = rand(num_pilots, 1) > shared_fraction;
        S(replace, user) = qpsk(randi(4, sum(replace), 1));
    end
    rng(old_rng);
end

function B = make_training_reference(cfg)
    time_phase = 2 * pi * mod((0:cfg.num_pilots - 1).', 4) / 4;
    space_phase = 2 * pi * (0:cfg.num_ris - 1) / cfg.num_ris;
    B = cfg.reference_amplitude * exp(1j * (time_phase + space_phase));
end
...
function eta = perturb_eta(eta_true, cfg)
    L = cfg.num_users;
    eta = eta_true;
    eta(1:2 * L) = eta(1:2 * L) + ...
        cfg.initial_angle_std_deg * pi / 180 * randn(2 * L, 1);
    eta(2 * L + 1:end) = eta(2 * L + 1:end) + ...
        cfg.initial_gain_std * randn(2 * L, 1);
    eta = project_eta(eta, cfg);
end
