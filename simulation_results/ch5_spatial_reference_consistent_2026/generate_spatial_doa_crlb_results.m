%% Spatial-reference DOA RMSE and CRLB simulation
...
fprintf('\nSpatial four-phase DOA/CRLB simulation\n');
...
export_publication_figure(fig, result_dir, 'fig_spatial_doa_rmse_crlb');
...
function phase_reference = make_random_reference_phase(num_pilots, num_ris, seed)
    old_rng = rng;
    rng(seed, 'twister');
    phase_row = exp(1j * (pi / 2) * randi([0, 3], 1, num_ris));
    phase_reference = repmat(phase_row, num_pilots, 1);
    rng(old_rng);
end

function phase_reference = make_uniform_reference_phase(num_pilots, num_ris)
    space_phase = (pi / 2) * mod(0:num_ris - 1, 4);
    phase_reference = repmat(exp(1j * space_phase), num_pilots, 1);
end
