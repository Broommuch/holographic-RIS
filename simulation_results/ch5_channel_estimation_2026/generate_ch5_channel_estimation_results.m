...
% This script is a publication-oriented derivative of the latest verified
% DOA/channel script:
%   ../01_核心_全息收发联合仿真/02_v2波形域与符号级_2026/tx_and_rx_test5_2.m
%
...
%
% Outputs are written to ./results in EPS, PNG, FIG, CSV, and MAT formats.
...
for i_snr = 1:num_snr
    ...
    for mc = 1:cfg.num_monte_carlo
        ...
        doa_sq_error(mc, :) = (theta_hat_deg - cfg.theta_true_deg).^2 + ...
            (phi_hat_deg - cfg.phi_true_deg).^2;
    end
...
export_publication_figure(fig, result_dir, 'fig_channel_nmse_snr');

fig = publication_figure([100, 100, 540, 390]);
semilogy(snr_dB_vec, max(doa_rmse_snr_deg, 1e-3), 'o-', ...
    'LineWidth', 1.5, 'MarkerSize', 6);
hold on;
...
legend('Gauss--Newton estimate', 'CRLB', 'Location', 'southwest');
xlim([snr_dB_vec(1), snr_dB_vec(end)]);
export_publication_figure(fig, result_dir, 'fig_doa_rmse_crlb_snr');
