%% Chapter V-D end-to-end symbol detection with estimated CSI
% This self-contained publication driver combines the verified building blocks
% used in the two preceding simulation subsections:
...
%
% Training and data observations both follow the post-detection model in
% main.tex:
%   z = |G*x+b|^2 + w,  w ~ N(0,sigma_w^2 I).
%
% All figures and numerical tables used by Chapter V-D are written to ./results
% in EPS, PNG, FIG, CSV, and MAT formats.
...
ser_training_reference = zeros(num_designs, 1);
nmse_training_reference_dB = zeros(num_designs, 1);
...
for i_design = 1:num_designs
    ...
    for channel_trial = 1:cfg.num_channel_trials_sweep
        H_hat = estimate_channel_realization( ...
            pilot_symbols, B, H_true, cfg.training_reference_snr_dB, ...
            40000 + 100 * i_design + channel_trial, cfg);
        nmse_trials(channel_trial) = channel_nmse(H_hat, H_true);
        G_hat = repmat(H_hat, cfg.num_reference_states, 1);
        ser_trials(channel_trial) = simulate_ml_mismatched_ser( ...
            G_true, G_hat, b_data_baseline, combination_indices, ...
            symbol_candidates, cfg.training_reference_data_snr_dB, ...
            cfg.num_data_per_channel_sweep, ...
            41000 + 1000 * i_design + channel_trial);
    end
    ser_training_reference(i_design) = mean(ser_trials);
    nmse_training_reference_dB(i_design) = ...
        10 * log10(max(mean(nmse_trials), eps));
    fprintf('Training %-12s: NMSE %.2f dB, SER %.3e\n', ...
        reference_names{i_design}, nmse_training_reference_dB(i_design), ...
        ser_training_reference(i_design));
end

training_reference_table = table(string(reference_names(:)), ...
    nmse_training_reference_dB, ser_training_reference, ...
    'VariableNames', {'TrainingReference', 'Channel_NMSE_dB', ...
    'EndToEnd_ML_SER'});
writetable(training_reference_table, ...
    fullfile(result_dir, 'training_reference_end_to_end.csv'));

% Estimate one common channel ensemble, then change only the data reference.
common_channel_estimates = cell(cfg.num_channel_trials_sweep, 1);
for channel_trial = 1:cfg.num_channel_trials_sweep
    common_channel_estimates{channel_trial} = estimate_channel_realization( ...
        pilot_symbols, B_training_baseline, H_true, ...
        cfg.data_reference_training_snr_dB, ...
        50000 + channel_trial, cfg);
end

ser_data_reference = zeros(num_designs, 1);
for i_design = 1:num_designs
    ser_trials = zeros(cfg.num_channel_trials_sweep, 1);
    for channel_trial = 1:cfg.num_channel_trials_sweep
        ...
        ser_trials(channel_trial) = simulate_ml_mismatched_ser( ...
            ...
            combination_indices, symbol_candidates, ...
            cfg.data_reference_snr_dB, cfg.num_data_per_channel_sweep, ...
            51000 + 1000 * i_design + channel_trial);
    end
    ser_data_reference(i_design) = mean(ser_trials);
    fprintf('Data %-12s: d_min %.2f, SER %.3e\n', ...
        reference_names{i_design}, data_reference_dmin(i_design), ...
        ser_data_reference(i_design));
end

data_reference_table = table(string(reference_names(:)), ...
    data_reference_dmin(:), ser_data_reference, ...
    'VariableNames', {'DataReference', 'Normalized_dmin_E', ...
    'EndToEnd_ML_SER'});
writetable(data_reference_table, ...
    fullfile(result_dir, 'data_reference_end_to_end.csv'));

fig = publication_figure([100, 100, 560, 650]);
tiledlayout(2, 1, 'TileSpacing', 'compact', 'Padding', 'compact');
nexttile;
bar(plotting_ser(ser_training_reference, ...
    cfg.num_channel_trials_sweep * cfg.num_data_per_channel_sweep, ...
    cfg.num_users), 0.68, 'FaceColor', colors(1, :));
set(gca, 'YScale', 'log', 'XTick', 1:num_designs, ...
    'XTickLabel', reference_names, 'XTickLabelRotation', 18);
grid on;
box on;
ylabel('End-to-end ML SER');
ylim([2e-4, 1]);
title('(a) Training-phase reference');

nexttile;
bar(plotting_ser(ser_data_reference, ...
    cfg.num_channel_trials_sweep * cfg.num_data_per_channel_sweep, ...
    cfg.num_users), 0.68, 'FaceColor', colors(2, :));
set(gca, 'YScale', 'log', 'XTick', 1:num_designs, ...
    'XTickLabel', reference_names, 'XTickLabelRotation', 18);
grid on;
box on;
ylabel('End-to-end ML SER');
ylim([2e-4, 1]);
title('(b) Data-phase reference');
export_publication_figure(fig, result_dir, ...
    'fig_reference_design_end_to_end');

%% Save complete numerical workspace
save(fullfile(result_dir, 'chapter5_end_to_end_results.mat'), ...
    'cfg', 'snr_detector_dB', 'channel_nmse_detector_dB', ...
    'ser_perfect_ml', 'ser_perfect_gn', 'ser_perfect_gs', ...
    'ser_estimated_ml', 'ser_estimated_gn', 'ser_estimated_gs', ...
    'pilot_length_vec', 'channel_nmse_pilot_dB', 'ser_pilot_length', ...
    'actual_controlled_nmse_dB', 'ser_controlled_nmse', ...
    'reference_names', 'nmse_training_reference_dB', ...
    'ser_training_reference', 'data_reference_dmin', ...
    'ser_data_reference', 'H_true', 'B_training_baseline', ...
    'data_reference_vectors');
