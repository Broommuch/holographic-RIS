%% Chapter V-C perfect-CSI detection with an analytical SER union bound
...
cfg.num_algorithm_trials = 100000;
...
%% 1) Detector comparison and distance-spectrum bound under perfect CSI
b_optimized = reference_vectors(:, 5);
mu_qpsk_optimized = intensity_codebook( ...
    G_data, b_optimized, qpsk_candidates);

% Corollary 1 gives Q(d_ij/(2*sigma_w)) for each pairwise ML event.
% Weighting these events by the symbol Hamming distance and applying the
% union bound gives a direct analytical upper bound on SER.
[ser_union_bound, ser_union_bound_raw, pairwise_energy_distance, ...
    pairwise_symbol_hamming] = symbol_error_union_bound( ...
    mu_qpsk_optimized, qpsk_indices, snr_algorithm_dB);
...
algorithm_table = table(snr_algorithm_dB(:), ser_ml, ser_gn, ser_gs, ...
    ser_union_bound, ser_union_bound_raw, ...
    'VariableNames', {'SNR_dB', 'ML_SER', 'Projected_GN_SER', ...
    'Reference_Assisted_GS_SER', 'Analytical_SER_Union_Bound', ...
    'Unclipped_SER_Union_Bound'});
...
semilogy(snr_algorithm_dB, max(ser_union_bound, eps), 'k--', ...
    'LineWidth', 1.5);
grid on;
box on;
xlabel('SNR (dB)');
...
legend('ML', 'Projected GN', 'Reference-assisted GS', ...
    'Analytical union bound', ...
    'Location', 'southwest');
xlim([snr_algorithm_dB(1) - 0.5, snr_algorithm_dB(end) + 0.5]);
ylim([2e-4, 1]);
...
save(fullfile(result_dir, 'chapter5_perfect_csi_detection_results.mat'), ...
    ...
    'ser_union_bound', 'ser_union_bound_raw', ...
    'pairwise_energy_distance', 'pairwise_symbol_hamming', ...
...
function [bound, raw_bound, distance, hamming] = ...
        symbol_error_union_bound(mu, combination_indices, snr_dB)
    num_candidates = size(mu, 2);
    ...
    distance_squared = squared_norms.' + squared_norms - ...
        2 * real(mu' * mu);
    distance = sqrt(max(distance_squared, 0));
    ...
    for snr_index = 1:numel(snr_dB)
        noise_variance = rms_intensity_squared / ...
            10^(snr_dB(snr_index) / 10);
        pairwise_probability = 0.5 * erfc( ...
            distance / (2 * sqrt(2 * noise_variance)));
        pairwise_probability(1:num_candidates + 1:end) = 0;
        raw_bound(snr_index) = sum( ...
            hamming .* pairwise_probability, 'all') / ...
