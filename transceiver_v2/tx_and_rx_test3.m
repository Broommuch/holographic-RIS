...
%% ================= 接收端：导频辅助角度估计 =================
% 说明：
% 这里假设当前发送的是导频帧，因此接收端已知 symbols_tx。
% 接收端利用符号级能量矩阵 E_simo 来估计用户到达角 theta, phi。
...
h_true = generate_ris_steering_vector( ...
    RIS_row, RIS_col, theta_true, phi_true, d, lambda);
...
[theta_hat, phi_hat, metric_map] = estimate_user_angle_from_symbol_energy( ...
    E_simo, tx_bb_hi, ref_symbols_ris, rrc, sps, interp, ...
    RIS_row, RIS_col, d, lambda, ...
    sym_center_idx, win_len, theta_grid, phi_grid);

fprintf('\n========== Angle Estimation Result ==========\n');
fprintf('True theta = %.2f deg\n', theta_true * 180/pi);
fprintf('True phi   = %.2f deg\n', phi_true   * 180/pi);
fprintf('Est. theta = %.2f deg\n', theta_hat  * 180/pi);
fprintf('Est. phi   = %.2f deg\n', phi_hat    * 180/pi);
fprintf('Theta error = %.2f deg\n', (theta_hat - theta_true) * 180/pi);
fprintf('Phi error   = %.2f deg\n', (phi_hat - phi_true) * 180/pi);

%% ================= 可视化角度估计谱 =================

figure;
imagesc(phi_grid_deg, theta_grid_deg, 10*log10(metric_map / min(metric_map(:))));
set(gca, 'YDir', 'normal');
colorbar;
xlabel('\phi / deg');
ylabel('\theta / deg');
title('Angle Estimation Metric Map, normalized in dB');

hold on;
plot(phi_true*180/pi, theta_true*180/pi, 'wo', 'LineWidth', 2, 'MarkerSize', 8);
plot(phi_hat*180/pi, theta_hat*180/pi, 'rx', 'LineWidth', 2, 'MarkerSize', 10);
...
function [theta_hat, phi_hat, metric_map] = estimate_user_angle_from_symbol_energy( ...
    E_obs, tx_bb_hi, ref_symbols_ris, rrc, sps, interp, ...
    RIS_row, RIS_col, d, lambda, ...
    sym_center_idx, win_len, theta_grid, phi_grid)
%ESTIMATE_USER_ANGLE_FROM_SYMBOL_ENERGY
% 基于已知导频符号和符号级能量观测估计用户到达角。
%
% 观测模型：
%   E_{m,k} = mean_{n in window k} | h_m(theta,phi) x[n] + r_m[n] |^2
%
% 输入：
%   E_obs           : M_ris x N_sym，观测到的符号级能量
%   tx_bb_hi        : 已知导频符号生成的高采样率复基带波形
%   ref_symbols_ris : N_sym x M_ris，每个单元的参考符号
%   rrc, sps, interp: 参考信号成形参数
%   RIS_row, RIS_col: RIS 阵列尺寸
%   d, lambda       : 阵元间距和波长
%   sym_center_idx  : 每个符号中心索引
%   win_len         : 每个符号能量积分窗口长度
%   theta_grid      : theta 搜索网格，rad
%   phi_grid        : phi 搜索网格，rad
%
% 输出：
%   theta_hat       : 估计 theta
%   phi_hat         : 估计 phi
%   metric_map      : length(theta_grid) x length(phi_grid) 残差图
    ...
    if M_ris ~= RIS_row * RIS_col
        error('E_obs 的行数必须等于 RIS_row * RIS_col。');
    end

    % 预先生成所有 RIS 单元的参考基带波形，避免网格搜索时重复生成
    ref_bb_hi_mat = generate_reference_waveforms_ris( ...
        ref_symbols_ris, rrc, sps, interp, length(tx_bb_hi));
    ...
    for it = 1:length(theta_grid)
        ...
        for ip = 1:length(phi_grid)
            ...
            E_pred = predict_simo_symbol_energy_baseband( ...
                tx_bb_hi, h_cand, ref_bb_hi_mat, sym_center_idx, win_len);

            % 归一化残差，避免整体能量尺度影响过大
            residual = E_obs - E_pred;

            metric_map(it, ip) = norm(residual, 'fro')^2 / max(norm(E_obs, 'fro')^2, 1e-12);
        end
    end

    [~, idx_min] = min(metric_map(:));

    [idx_theta, idx_phi] = ind2sub(size(metric_map), idx_min);

    theta_hat = theta_grid(idx_theta);
    phi_hat   = phi_grid(idx_phi);
end


function E_pred = predict_simo_symbol_energy_baseband( ...
    tx_bb_hi, h_ris, ref_bb_hi_mat, sym_center_idx, win_len)
%PREDICT_SIMO_SYMBOL_ENERGY_BASEBAND
% 对给定候选阵列流形 h_ris，预测每个 RIS 单元、每个符号的能量。
%
% 使用复基带等效模型：
%   u_m[n] = h_m * x[n] + r_m[n]
...
%
% 输入：
%   tx_bb_hi       : 高采样率导频复基带波形，L x 1
%   h_ris          : M_ris x 1 候选阵列流形
%   ref_bb_hi_mat  : L x M_ris，每个单元的参考复基带波形
%   sym_center_idx : N_sym x 1 符号中心索引
%   win_len        : 符号积分窗口长度
%
% 输出：
%   E_pred         : M_ris x N_sym 预测符号能量

    tx_bb_hi = tx_bb_hi(:);
    h_ris = h_ris(:);

    [L_ref, M_ris] = size(ref_bb_hi_mat);

    L = min(length(tx_bb_hi), L_ref);

    tx_bb_hi = tx_bb_hi(1:L);
    ...
    for m = 1:M_ris
        ...
        u_m = h_ris(m) * tx_bb_hi + ref_bb_hi_mat(:,m);
...
%GENERATE_REFERENCE_WAVEFORMS_RIS
% 根据每个 RIS 单元的参考符号，生成成形和插值后的参考复基带波形。
%
% 输入：
%   ref_symbols_ris : N_sym x M_ris
%   rrc             : RRC 滤波器
%   sps             : 每符号采样点数
%   interp          : 插值倍数
%   target_len      : 输出波形目标长度
%
% 输出：
%   ref_bb_hi_mat   : target_len x M_ris
...
%EXTRACT_SYMBOL_ENERGY_FROM_WAVEFORM 从能量波形中提取每个符号平均能量
